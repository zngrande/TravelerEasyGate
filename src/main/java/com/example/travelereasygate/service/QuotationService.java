...
import com.example.travelereasygate.DAO.*;
...
public class QuotationService {
    ...
    public Quotation createQuotation(int ITID, int AID, int groupSize, int createdBy) {
        ...
        if (previous != null) {
            ...
            // 「報價定價規則」跟「NP／團費成本規則」各自有獨立的預設 (見 MarginSetting#defaultPricing/
            // defaultTier), 這兩組預設可能是完全不同的兩筆規則, 分開查、分開套用, 不再假設同一筆規則
            // 兩邊都要。
            MarginSetting defaultPricing = marginSettingDAO.findDefaultPricing(AID);
            if (defaultPricing != null) {
                q.setFormulaMode("preset");
                q.setMSID(defaultPricing.getMSID());
            }
            MarginSetting defaultTier = marginSettingDAO.findDefaultTier(AID);
            if (defaultTier != null) {
                q.setTierFormulaMode("preset");
                q.setTierMSID(defaultTier.getMSID());
            }
        }
