...
public class AiParseService {
     ...
     * 線控確認暫存資料無誤後, 轉成正式行程
     * (matchedPid 有值就直接連結公司 POI, 沒有就當自訂項目, 名稱先用 AI 解析出來的文字)
     * startDate 選填 —— 原本這裡固定寫死 LocalDate.now() 當出發日期, 但 AI 解析的原始文件裡通常
     * 沒有明確出發日期 (或跟今天完全無關), 硬塞今天的日期反而是錯的資訊, 使用者事後還要自己記得去改。
     * 改成讓使用者在確認匯入這一步就可以自己填 (或乾脆留空, 跟「建立新行程」手動流程的「出發日期 (選填)」
     * 欄位行為一致), 留空就是沒有出發日期, 之後可以再到行程基本資料編輯裡補。
     */
    public Itinerary confirmImport(int IPID, String title, String country, String region, LocalDate startDate) {
        AiImport aiImport = aiImportDAO.findById(IPID);
